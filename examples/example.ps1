...
$ret = Invoke-Command -Session $binarySession -ArgumentList $srcSxs -ScriptBlock {
    param($sxsPath)
    ...
    if (Set-AppSenseDesktopNowBasePath $Using:srcPath -TrackTime -Verbose) {
        $ret = Add-AppSenseDesktopNowComponent -Component ManagementCenter -SetupParams "PATCH=$($Using:srcPath)\Software\Products\ManagementServer64.msp" -TrackTime -Verbose
...
$ret = Invoke-Command -Session $sqlSession -ArgumentList $srcSxs -ScriptBlock {
    ...
    $ret = Set-AppSenseDesktopNowLogPath -Path $Using:srcLogPath -TrackTime -Verbose
    if ($ret) { $ret = Add-AppSenseDesktopNowAncillary -AncillaryPath "$($Using:srcPath)\Software\Prerequisites\SQLEXPR_x86_ENU.exe" -ArgList @("/ConfigurationFile=`"$($Using:srcPath)\Software\Prerequisites\sqlexpress.ini`"", "/InstanceName=`"$($Using:dbInstance)`"", '/SAPWD=Welcome21','/NPENABLED=1', '/TCPENABLED=1', '/HIDECONSOLE', '/Q') -TrackTime -Verbose }
...
$ret = Invoke-Command -Session $amcSession -ArgumentList "AMS-MODULE-DEFAULT" -ScriptBlock {
    ...
    $ret = Set-AppSenseDesktopNowLogPath -Path $Using:srcLogPath -TrackTime -Verbose
    if ($ret) { $ret = Initialize-AppSenseDesktopNowInstance -ProductName ManagementCenter -DatabaseServer $Using:dbServer -DatabaseName $dbName -ConfigurationCredentials $Using:configCreds -ServiceCredentials $Using:serviceCreds -CreateConfigurer -TrackTime -Verbose }
    return $ret
}
...
$ret = Invoke-Command -Session $empsSession -ArgumentList "EMPS-MODULE-DEFAULT" -ScriptBlock {
    ...
    Import-Module "$($Using:modPath)\Configuration\AppSenseDesktopNowConfiguration" -Force
    $ret = Set-AppSenseDesktopNowLogPath -Path $Using:srcLogPath -TrackTime -Verbose
    if ($ret) { $ret = Initialize-AppSenseDesktopNowInstance -ProductName PersonalizationAndBrowserInterface -DatabaseServer $Using:dbServer -DatabaseName $dbName -ConfigurationCredentials $Using:configCreds -ServiceCredentials $Using:serviceCreds -CreateConfigurer -TrackTime -Verbose }
    return $ret
}
...
$ret = Invoke-Command -Session $awsSession -ScriptBlock {
    ...
    if ($ret) { $ret = Connect-AppSenseManagementServer -Server $([System.Net.Dns]::GetHostName()) -UseCurrentUser -TrackTime -Verbose }
